    ...
    public class EventBusRabbitMQConsumer
    {
        ...
        private async void ReceivedEvent(object sender, BasicDeliverEventArgs e)
        {
            if (e.RoutingKey == EventBusConstants.BasketCheckoutQueue)
            {
                ...
                //Messages don't come from UI client via controller, instead it comes from EventBusRabbitMQ
                var result = await _mediator.Send(command); // CheckoutOrderHandler will receive and handle the command message
            }
        }
